...
const AdminPanel = () => {
  ...
  const [activeItem, setActiveItem] = useState('Company'); // Track active item
  ...
  return (
    <div className="flex flex-col h-screen bg-yellow-100">
      {/* Navbar */}
      <header className="bg-pink-900 text-slate-900 p-4 flex justify-between items-center">
        <h1 className="text-xl text-white font-serif">Admin Panel</h1>
      ...
      <div className="flex flex-1">
        ...
        <nav className={`bg-blue-200 text-black w-64 p-4 transition-transform duration-300 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 md:block`}>
          <ul>
            {['Company', 'Users', 'Flights', 'Logout'].map((item) => (
              <NavLink to={item === 'Company' ? 'companytable' : item === 'Users' ? 'users' : item === 'Flights' ? 'flights' : 'logout'}>
                <li
                  key={item}
